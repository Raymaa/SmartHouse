 ...
 * Description:
 */
public class Aty_ShowWetData extends BaseActivity<UiInterface, DataPresenter<UiInterface>> implements UiInterface {
    ...
    private void initPicChart(int num) {
        ...
        if (num > 50) {
            ...
            sliceValue = new SliceValue((float)num, Color.parseColor("#3f4169E1")).setLabel("");
            values.add(sliceValue);
            sliceValue = new SliceValue((float)100-num,Color.parseColor("#ff4169E1")).setLabel("");
            values.add(sliceValue);
        }else{
            sliceValue = new SliceValue((float)num, Color.parseColor("#ff4169E1")).setLabel("");
            values.add(sliceValue);
            sliceValue = new SliceValue((float)100-num,Color.parseColor("#3f4169E1")).setLabel("");
            values.add(sliceValue);
        }
...
//      line.setHasLabelsOnlyForSelected(true);//点击数据坐标提示数据（设置了这个line.setHasLabels(true);就无效）
        ...
        line.setColor(Color.parseColor("#ff4169E1"));
        lines.add(line);
...
//        固定Y軸高度，設置x軸的移動
    ...
    public void ReadCo2(int[] values, String[] times) {
        ...
        avg/=i;
        tv_avg.setText("平均湿度："+avg+"%");
        tv_which.setText("室内湿度");
